...
async def dankmemer(ctx, param=None):
    ...
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        ...
        Execard.dankmemer = False
        await ctx.send("Dankmemer Successfully **disabled**", delete_after=0.1)
    while Execard.dankmemer is True:
        try:
            count += 1 
...
@Execard.command(aliases=["bump", "autobump"])
async def autobumper(ctx, param=None):
    await ctx.message.delete()
    count = 0
    if param is None:
        await ctx.send("Please specify True/false.", delete_after=0.1)
    ...
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
            await ctx.send("You can't bind Autobumper to a DM or GC", delete_after=0.1)
            return
        else:
            Execard.autobumper = True
            await ctx.send("Autobumper Successfully bound to `" + ctx.channel.name + "`", delete_after=0.1)
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Execard.autobumper = False
        await ctx.send("Autobumper Successfully **disabled**", delete_after=0.1)
    while Execard.autobumper is True:
        try:
            count += 1 
            await ctx.send('!d bump', delete_after=0.1)           
...
@Execard.command(aliases=["9/11", "911", "terrorist", "best-day"])
...
@Execard.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "Execard LOL"
        name = ""
...
@Execard.command(aliases=["fakename"])
...
@Execard.command(aliases=["Execardcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Execard_Execard_comment.png"))
    except:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(str(res['message'])) as resp:
                        image = await resp.read()
                with io.BytesIO(image) as file:
                    await ctx.send(file=discord.File(file, f"Execard_tweet.png"))
            except:
                await ctx.send(res['message'])


@Execard.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Execard_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@Execard.command(aliases=["deepfry"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Execard_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Execard_blur.png"))
        except:
...
@Execard.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Execard.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Execard.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Execard.user.relationships)
    for relationship in Execard.user.relationships:
...
@Execard.command(aliases=["changeregions", "changeregion", "regionschange"])
...
@Execard.command(aliases=["gcleave"])
...
@Execard.command(aliases=['serverbanner'])
...
async def testetb(ctx):
    ...
    for user in users:
        try:
            await ctx.guild.ban(user, reason="Execard")
        except:
            pass


@Execard.command(aliases=["xavier"])
async def invite(ctx):
    await ctx.message.delete()
    await ctx.send('https://discord.com/oauth2/authorize?scope=bot&client_id=705544735540117526&permissions=8')


@Execard.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Execard.command_prefix = str(prefix)
...
async def cat(ctx): # b'\xfc'
    ...
    if cat_key == '':
        ...
        try:
            ...
            r = req.json()
            em = discord.Embed(title="Random Cat image", color=RandomColor())
...
async def dog(ctx): # b'\xfc'
    ...
    em = discord.Embed(title="Random Dog image", color=RandomColor())
...
async def decode(ctx, string): # b'\xfc'+
    ...
    await ctx.message.edit(content= (decoded_stuff))
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Execard_cumslut.gif"))
    except:
        em = discord.Embed()
...
@Execard.command(aliases=['tether'])
async def usdt(ctx):
    await ctx.message.delete()
...
@Execard.command(aliases=['bitcoincash'])
async def bch(ctx):
    await ctx.message.delete()
...
@Execard.command(aliases=['litecoin'])
async def ltc(ctx):
    await ctx.message.delete()
...
async def panda(ctx):
    ...
    embed = discord.Embed(color=RandomColor())
    embed.set_author(name="Random Panda Image", icon_url="https://cdn.freebiesupply.com/logos/large/2x/panda-7-logo-png-transparent.png") 
